...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    char *activation_s = option_find_str(options, "activation", "logistic"); // 활성함수가 무엇인지 찾음.
    ACTIVATION activation = get_activation(activation_s); //찾은 활성함수를 지정.
...
network *parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        options = s->options;
        layer l = {0};
        LAYER_TYPE lt = string_to_layer_type(s->type); // 어떤 layer인지 찾는다.
...
#endif
    if(workspace_size){
        printf("workspace_size=%.2lfMB\n", (double)workspace_size / 1048576.0);
#ifdef GPU
        if(gpu_index >= 0){
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        printf("######## %d/'s layer biases ########\n", i);
        for(int z = 0; z < l.n; z++){
            printf("[%d]: %f\n", z, l.biases[z]);
        }

        printf("######## %d/'s layer weights ########\n", i);
